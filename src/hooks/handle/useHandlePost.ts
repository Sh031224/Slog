...
const useHandlePost = () => {
  ...
  const [value, setValue] = useState<CreatePostDTO>({
    ...
    thumbnail: data.post.thumbnail.replace(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/static/`, ""),
  ...
  const handleContentTab = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
  ...
  const saveCallback = (isTemp: boolean, createdIdx?: number) => {
    if (isTemp || isNew) {
